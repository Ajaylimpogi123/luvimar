		...
		foreach ($pid as $prid)
		{
			...
			if (isset($_POST['qty_' . $prid]) && $_POST['qty_' . $prid] > 0) 
			{
				...
				foreach($con as $op)
				{
					...
					# UPDATE tbl_cart_receive
					$up = $conn->prepare("UPDATE tbl_cart_receive SET ct_qty = '$op'
		...
		foreach ($pid as $prid)
		{
			...
			if (isset($_POST['cost_' . $prid]) && $_POST['cost_' . $prid] > 0) 
			{
				...
				foreach($con as $op)
				{
					...
					# UPDATE tbl_cart_receive
					$up = $conn->prepare("UPDATE tbl_cart_receive SET ct_cost = '$op'
								WHERE ct_id = '$prid' AND user_id = '$userId'");
					$up->execute();
				}
			}else{}		
		...
		foreach ($pid as $prid)
		{
			...
			if (isset($_POST['price_' . $prid]) && $_POST['price_' . $prid] > 0) 
			{
				...
				foreach($con as $op)
				{
					...
					# UPDATE tbl_cart_receive
					$up = $conn->prepare("UPDATE tbl_cart_receive SET ct_price = '$op'
								WHERE ct_id = '$prid' AND user_id = '$userId'");
					$up->execute();
				}
			}else{}		
