...
if (!defined('WEB_ROOT')) {
	...
	$sql2 = $conn->prepare("SELECT *
				FROM tbl_order
					WHERE od_id = '$oi' AND is_deleted != '1' AND is_charge = '1'");
	$sql2->execute();
	$sql2_data = $sql2->fetch();
	
	$bal = $conn->prepare("SELECT SUM(amount_paid) as total_pay FROM tr_payment WHERE cust_id = '$id' AND od_id = '$oi'");
	$bal->execute();
	$bal_data = $bal->fetch();
	$total_pay = $bal_data['total_pay'];
	$total_balance = $sql2_data['od_total_amt_due'] - $total_pay;
...
?>
		...
		<div class="row-fluid sortable">
				...
				<form class="form-horizontal" method="post" action="processPayment.php" enctype="multipart/form-data" name="form" id="form">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-edit"></i> Update Payment - <small style="font-size:14px;">Customer</small></h2>
					...
					<div class="box-content">
						...
						<table class="table table-striped table-bordered">
						  <tbody>
							  <tr>
								  ...
								  <td><b>PO #</b></td>
								  <td> : </td>
								  <td><?php echo utf8_encode($sql2_data['po_num']); ?></td>
								  <td></td>
								  <td><b>Invoice #</b></td>
								  <td> : </td>
								  <td><?php echo utf8_encode($sql2_data['invoice_num']); ?></td>
							  </tr>
							  <tr>
								  <td><b>Amount Receivable</b></td>
								  <td> : </td>
								  <td>Php <?php echo number_format($sql2_data['od_total_amt_due'], 2); ?></td>
						...
						<a class="btn btn-primary" href="index.php?view=detail&id=<?php echo $id; ?>">Back</a>
					</div>
				</form>
				</div>
		</div>
		...
		<div class="row-fluid sortable">		
				<div class="box span12">
					<div class="box-header well" data-original-title>
						<h2><i class="icon-certificate"></i> Payment History - <small style="font-size:14px;">Customer</small></h2>
						<div class="box-icon">
					...
					<div class="box-content">
						<table class="table table-striped table-bordered">
						  ...
						  <tbody>
							<?php
								$sql3 = $conn->prepare("SELECT * FROM tr_payment p, bs_customer c
											WHERE p.is_deleted != '1' AND c.cust_id = p.cust_id AND p.od_id = '$oi'
													ORDER BY p.date_paid, p.pay_id DESC");
								$sql3->execute();
								if($sql3->rowCount() > 0)
								{
									$ctr = 1;
							...
							?>
										...
										<tr>
											...
											<td style="color:<?php echo $sql3_data['bcolor']; ?>;">Php <?php echo number_format($balance, 2); ?></td>
										</tr>
