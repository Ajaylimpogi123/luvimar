...
?>
	...
	<div id="myTabContent" class="tab-content">
		<?php
			$productsPerRow = 4; $columnWidth = (int)(100 / $productsPerRow);			
			$prod = $conn->prepare("SELECT * FROM tbl_product WHERE pd_keyword LIKE '%$keyword%' AND is_deleted != '1' ORDER BY pd_name ASC");
			$prod->execute();
		?>
			<h4><font color="#000000" size="2">Search result(s) for:</font> &nbsp; <b><?php echo $keyword; ?></b></h4>
			<br />
			...
			?>
						<td valign="top">	
							...
							<tr><td align="center">
								...
								<?php echo $catname; ?><br />
								<a href="detail.php?id=<?php echo $prod_data['pd_id']; ?>" class="nyroModal"><?php echo $prod_data['pd_name']; ?></a><br />
								<br />
			...
			<?php
						if ($i % $productsPerRow == $productsPerRow - 1) { echo '</tr>'; }
						$i += 1;
						if ($i % $productsPerRow > 0) { echo '<td colspan="' . ($productsPerRow - ($i % $productsPerRow)) . '">&nbsp;</td>'; }
					} // End While
				}else{}
			?>
			</table>			
