...
{
	include 'database.php';
	$sql = $conn->prepare("SELECT cat_id, cat_name, cat_image
	        FROM tbl_category
    ...
    while ($sql_data = $sql->fetch()) {
		...
		if ($sql_data['cat_image']) {
			$sql_data['cat_image'] = WEB_ROOT . 'images/category/' . $sql_data['cat_image'];
		} else {
...
{
	include 'database.php';
    $sql = $conn->prepare("SELECT cat_id, cat_parent_id, cat_name, cat_image, cat_description
	        FROM tbl_category
