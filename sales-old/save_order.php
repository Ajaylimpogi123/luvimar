...
$sql = $conn->prepare("INSERT INTO tbl_order(cust_id, invoice_num, dr_num, po_num, person_received, terms_in_days, customer_name, od_amount_due, od_discount, percent_discount, od_total_amt_due, od_cost, od_payment, od_change, dc_id, od_date, od_date_1, od_paid_date, date_due,  is_paid, is_charge, released_by, remarks)
											VALUES ('$custid', '$ordernum', '$drnum', '$ponum', '$recby', '$tidays', '$custname', '$amtdue', '$disc', '$perdisc', '$total_due', '$cost', '$payment', '$total_change', '$discId', '$today_date1', '$today_date2', '$paiddate', '$newDate',  '$paid', '$charge', '$userId', '$remarks')");
$sql->execute();
	...
	for ($i = 0; $i < $sql1->rowCount(); $i++) {
		...
		if ($sql1_data['is_type'] == 3) {
			...
			if ($crt_qty != 0) {
				...
				$sql = $conn->prepare("INSERT INTO tbl_order_item(od_id, pd_id, od_qty, od_price, od_cost, pd_qty_left, pd_type,branch_id)
										VALUES ($orderId, '$crt_pid', '$crt_qty', '$prc', '$cost', '$bal_qty', 'pc','$custid')");
				$sql->execute();
