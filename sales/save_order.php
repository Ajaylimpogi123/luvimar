...
$daterefnum = date("mdy-hi", strtotime($today_date1));

$refnum = "REF-" . $daterefnum;
...
$brnId = 1006; // Default Branch ID (Luvimar Main Branch Bacolod)
...
if ($orderId) {
	for ($i = 0; $i < $sql1->rowCount(); $i++) {
		...
		$crt = $conn->prepare("SELECT * FROM tbl_cart WHERE ct_id = '$crt_cid'");
		$crt->execute();
		$crt_data = $crt->fetch();
		...
		if ($sql1_data['is_type'] == 3) {
			...
			$crt_item_price = $crt_data['ct_price'];
			...
			if ($crt_qty != 0) {
				...
				if (!empty($custid)) {
					...
					$expiration = date("F j, Y", strtotime($today_date2 . ' +2 years'));

					$up_pc = $conn->prepare("UPDATE tbl_product SET pc_qty = pc_qty - '$crt_qty', pd_expiration = '$expiration', is_sold = 1 WHERE pd_id = '$crt_pid'");
					$up_pc->execute();
					...
					$sql = $conn->prepare("INSERT INTO tbl_order_item(od_id, pd_id, cust_id, branch_id, od_qty, od_price, od_cost, pd_qty_left, pd_type, odi_description, odi_job_description)
											VALUES ($orderId, '$crt_pid', '$custid', '$brnId', '$crt_qty', '$crt_item_price', '$cost', '$bal_qty', 'pc', '$desc', '$jdesc')");
					$sql->execute();
