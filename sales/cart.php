			...
			<form action="index.php?view=customer" method="post" name="frmCart" id="frmCart">
				<table border="0" width="100%">
					...
					for ($i = 0; $i < $sql->rowCount(); $i++) {
						...
						if ($sql_data['is_type'] == 1) {
							$tp = "pc";
						} else if ($sql_data['is_type'] == 2) {
							$tp = "bx";
						} else {
							$tp = "cs";
						}
							...
							<script>
								...
								function toggleRadio(radio) {
									if (lastChecked === radio) {
										...
										// Create a hidden blank input to override value
										let hiddenInput = document.createElement("input");
										hiddenInput.type = "hidden";
										hiddenInput.name = radio.name;
										hiddenInput.value = "";
										radio.parentNode.appendChild(hiddenInput);
									} else {
										lastChecked = radio;
										// Remove any hidden blank input for this group
										let hiddenInputs = document.querySelectorAll(`input[type="hidden"][name="${radio.name}"]`);
