...
function uploadimage($inputName, $uploadDir)
{
	...
	return array('image' => $imagePath, 'thumbnail' => $thumbnailPath);
				...
				$check = $conn->prepare("SELECT * FROM bs_supplier
							WHERE company_name = '$sn' AND is_deleted != '1'");
				$check->execute();
				
				if($check->rowCount() > 0)
				{
					header('Location: index.php?view=add&error=' . urlencode('Supplier already exist!'));
				}
				else
				{
					
					/* Insert Customer */
					$sql = $conn->prepare("INSERT INTO bs_supplier (company_name, firstname, lastname, contactno, address, image, thumbnail, date_added, is_deleted)
								VALUES ('$sn', '$firstname', '$lastname', '$cn', '$address', '$mainImage', '$thumbnail', '$today_date1', '0')");
					$sql->execute();
					/* End Customer */
					...
					$log = $conn->prepare("INSERT INTO tr_log (action, description, category, action_by, log_action_date)
								VALUES ('Supplier added', '$sn', 'supplier', '$userId', NOW())");
					$log->execute();
					/* End Log */
