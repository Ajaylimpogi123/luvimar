<?php
$productsPerRow7 = 4;
$columnWidth7 = (int)(100 / $productsPerRow7);
			...
			<table width="100%" border="0" cellspacing="7" cellspacing="7">
				...
				if ($prod7->rowCount() > 0) {
					...
					while ($prod7_data = $prod7->fetch()) {
						...
						$catname7 = $cat7_data['cat_name'];

						if ($i % $productsPerRow7 == 0) {
							echo '<tr>';
						}
				?>
						<td valign="top">
				...
				<?php
						if ($i % $productsPerRow7 == $productsPerRow7 - 1) {
							echo '</tr>';
						}
						$i += 1;
						if ($i % $productsPerRow7 > 0) {
							echo '<td colspan="' . ($productsPerRow7 - ($i % $productsPerRow7)) . '">&nbsp;</td>';
						}
					} // End While
				} else {
				}
