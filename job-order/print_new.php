...
if ($joId) {
    ...
    if ($sql->rowCount() > 0) {
        ...
        $jol = $conn->prepare("SELECT * FROM tbl_jo_list_new jol, tbl_jo_items_new joi,  bs_customer cus, bs_user us 
        WHERE jol.jo_id = '$joId' AND joi.join_id = jol.join_id  AND jol.user_id = us.user_id AND cus.cust_id = joi.cust_id AND joi.is_submitted = '1'");
        $jol->execute();
